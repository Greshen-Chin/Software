      ...
      <div class="grid grid-cols-12 gap-6">
        ...
        <div class="col-span-12 lg:col-span-4 space-y-6">
          ...
          <div class="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
             <h4 class="font-bold text-slate-800 mb-6">Statistik Tugas</h4>
             <div class="flex items-center justify-center h-48 relative">
                <div class="w-40 h-40 rounded-full border-[12px] border-indigo-500 border-t-slate-100 rotate-45 flex items-center justify-center">
                   <div class="text-center -rotate-45">
                      <span class="text-2xl font-black block">{{ Math.round((store.stats.done / (store.tasks.length || 1)) * 100) }}%</span>
                      <span class="text-[10px] text-slate-400 uppercase font-bold">Progress</span>
                   </div>
                </div>
             </div>
             <div class="grid grid-cols-2 gap-4 mt-6">
                <div class="bg-green-50 p-3 rounded-xl text-center">
                  <span class="text-xs font-bold text-green-600 block uppercase">Done</span>
                  <span class="text-lg font-black">{{ store.stats.done }}</span>
                </div>
                <div class="bg-red-50 p-3 rounded-xl text-center">
                  <span class="text-xs font-bold text-red-600 block uppercase">Expired</span>
                  <span class="text-lg font-black">{{ store.stats.expired }}</span>
                </div>
             </div>
          </div>
        </div>
